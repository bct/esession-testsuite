...
class SessionNegotiation(session.Session):
  def show_help(self, msg):
    self.send('type "begin" to have me initiate a session (as described in XEP-0155).')
  ...
  def handle_message(self, msg):
    ...
    if self.status == 'requested':
      ...
      if self.status == 'agreed' and self.polite:
        ...
        self.send('message acknowledged.')
  
  handlers = { 'begin': do_begin,
               'help': show_help,
                                        }
